> ## ⚠️ このフォルダは旧版です。編集しないでください
>
> Salon AI OS の開発は専用リポジトリへ移動しました。
>
> **正となるリポジトリ： https://github.com/chainonjoli/salon-AI-OS**
>
> こちらのフォルダは移動前のスナップショットで、`admin.html`（管理画面）・`factory.html`
> （AIコンテンツファクトリー）・`presets.js`（業種プリセット）・`intro.html`（販売用紹介ページ）
> を含みません。開発を引き継ぐ方は、新リポジトリの `HANDOFF.md` を最初に読んでください。
